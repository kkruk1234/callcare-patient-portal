...
def main() -> int:
    ...
    for line in lines:
        ...
        if marker in line and not changed:
            # use the exact indentation of the marker line
            indent = line[:len(line) - len(line.lstrip())]
            # add debug block at same indentation level as the marker line
            block = [
    ...
    bak = P.with_suffix(P.suffix + f".bak_EVIDDEBUGV2_{ts}")
